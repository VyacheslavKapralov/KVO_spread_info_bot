...
async def set_type_spread_alert(callback: types.CallbackQuery, state: FSMContext):
    ...
    if data['type_alert'] == 'line_alert':
        ...
        task = asyncio.create_task(signal_bb(data, callback.message, monitor_id, spread_monitor))
        await spread_monitor.add_monitor(callback.from_user.id, monitor_id, task, data)
        await callback.message.answer(BotAnswers.start_monitoring(monitor_id), reply_markup=menu_chancel())
...
async def set_maximum_line_alert(message: types.Message, state: FSMContext):
    ...
    await message.answer(BotAnswers.start_monitoring(monitor_id), reply_markup=menu_chancel())
...
async def show_monitor_controls(message: types.Message):
    monitors = await spread_monitor.get_user_monitors(message.from_user.id)
    if monitors:
        await message.answer(BotAnswers.monitoring(), reply_markup=menu_monitor_control())
    else:
        await message.answer(BotAnswers.not_active_monitoring(), reply_markup=main_menu())
...
async def command_chancel(message: types.Message, state: FSMContext):
    ...
    if current_state:
        await state.finish()
    await show_monitor_controls(message)
...
async def register_handlers_alerts(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(stop_all_monitors, lambda callback: callback.data == 'stop_all')
