...
from telegram_api.essence.state_machine import EMA
...
async def get_spread(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        await message.answer(BotAnswers.spread_ema_moex(data['tool_1'], data['tool_2'], data['spread_type']))
        ...
        ema = round(calculate_ema(df)['ema'].iloc[-1], 3)
        logger.info(f"EMA {data['tool_1']}/{data['tool_2']} = {ema}")
        data['ema'] = ema
...
async def sending_signal(message: types.Message, data: dict):
    await message.answer(f"EMA спреда {data['tool_1']} к {data['tool_2']} ({data['spread']}): {data['ema']}",
...
def register_handlers_command_ema(dp: Dispatcher):
    ...
    # dp.register_message_handler(exponential_ma, Text(equals=['EMA', 'ema', 'экспоненциальная']))
    dp.register_callback_query_handler(set_spread_type, state=EMA.spread_type)
