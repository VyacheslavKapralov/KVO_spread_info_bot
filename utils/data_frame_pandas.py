...
@logger.catch()
async def get_dataframe_3(data: dict, data_frame_1: pd.DataFrame, data_frame_2: pd.DataFrame):
    ...
    if data['spread_type'] == 'money':
        data_frame_3['Open'] = merged_df['Open_1'] - merged_df['Open_2']
        data_frame_3['High'] = merged_df['High_1'] - merged_df['High_2']
        data_frame_3['Low'] = merged_df['Low_1'] - merged_df['Low_2']
        data_frame_3['Close'] = merged_df['Close_1'] - merged_df['Close_2']
    elif data['spread_type'] == 'percent':
        data_frame_3['Open'] = (merged_df['Open_1'] / merged_df['Open_2'] - 1) * 100
        data_frame_3['High'] = (merged_df['High_1'] / merged_df['High_2'] - 1) * 100
        data_frame_3['Low'] = (merged_df['Low_1'] / merged_df['Low_2'] - 1) * 100
        data_frame_3['Close'] = (merged_df['Close_1'] / merged_df['Close_2'] - 1) * 100
    return data_frame_3
...
@logger.catch()
async def add_dataframe_spread_bb(data: dict, candle_interval: str, deviation: int, period: int):
    data_frame = await create_dataframe_spread(data, candle_interval)
...
@logger.catch()
async def create_dataframe_spread(data: dict, candle_interval: str):
